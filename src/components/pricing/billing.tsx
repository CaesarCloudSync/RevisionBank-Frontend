...
function CheckoutForm(props:any) {
  ...
  const storeSubscriptionData = async (subscription:string,start_date:string,end_date:string,token:any) => {
    ...
    const response:any = await axios.post(`https://revisionbankbackendsql-aoz2m6et2a-uc.a.run.app/storesubscription`, json,config); // Send login post request.
  }
  const payMoney = async (e:any) => {
    ...
    const getClientSecret = async (price:any) => { 
        ...
        const response:any = await axios.post(`https://revisionbankbackendsql-aoz2m6et2a-uc.a.run.app/revisionbankstripepayment`, json,config); // Send login post request.
