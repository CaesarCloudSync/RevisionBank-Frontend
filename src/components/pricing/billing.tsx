...
function CheckoutForm(props:any) {
  ...
  const storeSubscriptionData = async (subscription:string,start_date:string,end_date:string,token:any) => {
    ...
    const response:any = await axios.post(`http://127.0.0.1:8080/storesubscription`, json,config); // Send login post request.
  }
  const payMoney = async (e:any) => {
    ...
    const getClientSecret = async (price:any) => { 
        ...
        const response:any = await axios.post(`http://127.0.0.1:8080/revisionbankstripepayment`, json,config); // Send login post request.
