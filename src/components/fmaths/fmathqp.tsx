...
export default function FmathQP (){
    ...
    const sendApi = async (e:any) => {
      ...
      const config = {headers: {Authorization: `Bearer ${token.token}`,}}
      //console.log(email)
      const response = await axios.post("https://palondomus-revb-backend.hf.space/fmathsqp",{"furthermaths":{"email":email,"furthermathsbook": bookidselect,"furthermathstopic":topicselect.label,"platform": "web"}},config)
