...
export default function FmathSB(){
    ...
    const sendApi = async (e:any) => {
        ...
        if ((email  === '' && emailcount !== 0 ) ||  furthermathsbook === '' || furthermathsyear === '' || furthermathsexerciesNum === ''){  // Checks if all fields are filled, and is tolerent to the email not being filled
          ...
          const config = {headers: {Authorization: `Bearer ${token.token}`,}}
          try{
          const response:any = await axios.post("https://revisionbankbackend-aoz2m6et2a-uc.a.run.appfmathsb",{"furthermathsb":{"email":email,"furthermathsbbook": furthermathsbook,"furthermathsbyear":furthermathsyear.label,"furthermathsbexercise":furthermathsexerciesNum,"platform":"web"}},config)
