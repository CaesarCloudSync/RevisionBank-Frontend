...
export default function AccountPage(){
    ...
    const getaccountinfo = async (token:any) => {
        ...
        const response:any = await axios.get(`http://192.168.0.22:8080/getaccountinfo`,config); // Send login post request.
    ...
    const getstudentsubscriptions = async (token:any) => {
        const config = {headers: {Authorization: `Bearer ${token}`,}}
        const response:any = await axios.get(`http://192.168.0.22:8080/getstudentsubscriptions`,config)
