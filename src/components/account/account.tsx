...
export default function AccountPage(){
    ...
    const getaccountinfo = async (token:any) => {
        ...
        const response:any = await axios.get(`https://revisionbankbackend-qqbn26mgpa-uc.a.run.app/getaccountinfo`,config); // Send login post request.
    ...
    const getstudentsubscriptions = async (token:any) => {
        const config = {headers: {Authorization: `Bearer ${token}`,}}
        const response:any = await axios.get(`https://revisionbankbackend-qqbn26mgpa-uc.a.run.app/getstudentsubscriptions`,config)
