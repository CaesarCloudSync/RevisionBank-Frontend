...
export default function AccountPage(){
    ...
    const getaccountinfo = async (token:any) => {
        ...
        const response:any = await axios.get(`http://127.0.0.1:8080/getaccountinfo`,config); // Send login post request.
    ...
    const getstudentsubscriptions = async (token:any) => {
        const config = {headers: {Authorization: `Bearer ${token}`,}}
        const response:any = await axios.get(`http://127.0.0.1:8080/getstudentsubscriptions`,config)
