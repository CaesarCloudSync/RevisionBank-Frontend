...
export default function ManageAccounts(props:any){
    ...
    const deletestudentaccount = async (e:any,studentemail:any) => {
        ...
        const response:any = await axios.post(`http://127.0.0.1:8080/deletestudentaccount`,{"studentemail":studentemail},config); // Send login post request.
        const studentresponse:any = await axios.get(`http://127.0.0.1:8080/getstudentsubscriptions`,config)
        props.setStudentAccountInfo(studentresponse.data.result)
        //console.log(response.data)
            //const response:any = await axios.post(`http://127.0.0.1:8080/setstudentsubscriptions`,json,config)
    ...
    const changestudentpassword = async (e:any) => {
        ...
        const config = {headers: {Authorization: `Bearer ${props.token}`,}}
        //console.log(newpassword)
        const response:any = await axios.put(`http://127.0.0.1:8080/changestudentpassword`,{"studentemail":studentemailchangepass,"password":newpassword},config); // Send login post request.
        const studentresponse:any = await axios.get(`http://127.0.0.1:8080/getstudentsubscriptions`,config)
        props.setStudentAccountInfo(studentresponse.data.result)
